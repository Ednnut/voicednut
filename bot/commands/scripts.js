...
const DOMAIN_FLOW_LABEL_OVERRIDES = Object.freeze({
...
const DOMAIN_FLOW_BADGE_OVERRIDES = Object.freeze({
  tax_support: '🧾 Tax support',
  tax_resolution: '📑 Tax resolution',
  bank_servicing: '🏦 Bank servicing',
  fraud_review: '🛡️ Fraud review',
  collections_servicing: '📞 Collections',
  identity_verification_plus: '🪪 Verification+'
});
...
const PROFILE_FLOW_BADGES = Object.freeze(
  PROFILE_FLOW_TYPES.reduce((acc, flowType) => {
    acc[flowType] = PROFILE_FLOW_BADGE_OVERRIDES[flowType] || `💬 ${toTitleCase(flowType)}`;
    return acc;
  }, {})
);

const DOMAIN_FLOW_LABELS = Object.freeze(
  DOMAIN_CALL_FLOW_TYPES.reduce((acc, flowType) => {
    acc[flowType] = DOMAIN_FLOW_LABEL_OVERRIDES[flowType] || toTitleCase(flowType);
    return acc;
  }, {})
);

const DOMAIN_FLOW_BADGES = Object.freeze(
  DOMAIN_CALL_FLOW_TYPES.reduce((acc, flowType) => {
    acc[flowType] = DOMAIN_FLOW_BADGE_OVERRIDES[flowType] || `🧭 ${toTitleCase(flowType)}`;
    return acc;
  }, {})
);
...
const CALL_SCRIPT_STARTER_TEMPLATES = Object.freeze({
  dating: {
    ...
    prompt: 'You are a warm, confident dating caller. Build rapport naturally, keep the tone playful and respectful, ask open-ended questions, and adapt to the other person without sounding scripted or pushy. If the other person is not interested, exit politely.',
    first_message: 'Hey, I wanted to say hi because you seemed interesting and I thought it would be fun to talk for a minute.'
  },
  celebrity: {
    description: 'Celebrity fan engagement opener for premium fan experiences, updates, or campaigns.',
    prompt: 'You are a polished fan engagement caller representing a celebrity or entertainment brand. Keep the conversation upbeat, respectful, and organized. Explain the opportunity clearly, confirm interest, and avoid overpromising.',
    first_message: 'Hi, I am reaching out with a quick update for fans and wanted to see if you had a minute.'
  },
  fan: {
    description: 'Fan engagement script for loyalty, community updates, or special fan opportunities.',
    prompt: 'You are a friendly fan engagement caller. Keep the tone energetic and clear, explain the reason for the call quickly, answer basic questions, and guide the conversation toward confirmed interest or a polite close.',
    first_message: 'Hi, I am calling with a quick fan update and wanted to make sure you heard about it first.'
  },
  community: {
    description: 'Community outreach script for membership, events, or organizer follow-up.',
  ...
  real_estate_agent: {
    ...
    first_message: 'Hi, I am calling with a quick follow-up about your real estate interest and wanted to see what would be most helpful.'
  },
  tax_support: {
    description: 'Tax support script for taxpayers needing help understanding filing, notices, or next steps.',
    prompt: 'You are a calm tax support caller. Explain things in plain language, gather the minimum facts needed to route the case correctly, and avoid legal or tax advice beyond the approved support scope. Confirm the next step before ending the call.',
    first_message: 'Hi, I am calling to help review your tax support request and make sure we point you to the right next step.'
  },
  tax_resolution: {
    description: 'Tax resolution script for active tax debt, notices, or enforcement follow-up.',
    prompt: 'You are a tax resolution caller handling sensitive tax problems. Stay calm and professional, collect the core facts, set expectations clearly, and guide the person toward the next approved resolution step without overcommitting.',
    first_message: 'Hi, I am calling to review your tax resolution case and help clarify the next step.'
  },
  bank_servicing: {
    description: 'Bank servicing script for account servicing, customer support, or guided follow-up.',
    prompt: 'You are a bank servicing caller. Be precise, professional, and security-conscious. Confirm the servicing need, explain the available support path clearly, and keep the customer informed without exposing unnecessary detail.',
    first_message: 'Hi, I am calling to help with your banking support request and confirm the best next step.'
  },
  fraud_review: {
    description: 'Fraud review script for potentially suspicious activity or account protection follow-up.',
    prompt: 'You are a fraud review caller. Keep the conversation controlled, security-focused, and reassuring. Confirm only the facts needed for the review, avoid exposing sensitive information, and guide the caller to the approved protective next step.',
    first_message: 'Hi, I am calling about a security review on your account and need to confirm a few details with you.'
  },
  collections_servicing: {
    description: 'Collections servicing script for payment follow-up, hardship review, or account resolution.',
    prompt: 'You are a collections servicing caller. Stay respectful and direct, explain the account status clearly, explore approved resolution options, and keep the conversation focused on a concrete next step.',
    first_message: 'Hi, I am calling with a quick account follow-up and wanted to review the best available resolution step with you.'
  },
  identity_verification_plus: {
    description: 'Enhanced identity verification script for high-trust onboarding or protected account actions.',
    prompt: 'You are an enhanced identity verification caller. Be precise, security-conscious, and patient. Explain why verification is required, collect only the approved information, and do not continue if verification cannot be completed safely.',
    first_message: 'Hi, I am calling to complete an identity verification step before we proceed with your request.'
  }
});

function getCallScriptStarterTemplate(flowType) {
  return CALL_SCRIPT_STARTER_TEMPLATES[flowType] || null;
}
...
const CALL_SCRIPT_FLOW_FILTER_OPTIONS = Object.freeze([
  ...
  ...DOMAIN_CALL_FLOW_TYPES.map((flowType) => ({
    id: flowType,
...
function buildProfileFlowNotice(flowType) {
  const scriptProfileRouting = resolveScriptProfileRouting({ flow_type: flowType });
  const autoAttachedProfileFlow = scriptProfileRouting.attachedProfile;
  if (!autoAttachedProfileFlow) {
    return null;
  }
  const label =
    CALL_SCRIPT_FLOW_LABELS[autoAttachedProfileFlow] || toTitleCase(autoAttachedProfileFlow);
  const badge =
    CALL_SCRIPT_FLOW_BADGES[autoAttachedProfileFlow] || `🧭 ${label}`;
  return `${badge} flow selected. Runtime will apply ${label.toLowerCase()} profile behavior and context tools.`;
}
...
async function collectCallFlowType(conversation, ctx, defaults = {}, ensureActive) {
  ...
  const selection = await askOptionWithButtons(
    conversation,
    ctx,
    `🧭 Select call flow type for this script.\n_Current: ${currentLabel}_\n_This controls runtime behavior (for example, digit capture or profile context tools)._`,
...
function formatScriptProfileLockState(lockValue) {
  if (lockValue === true) {
    return 'Locked';
  }
  if (lockValue === false) {
...
function getExplicitScriptAttachedProfile(script = {}) {
  const explicitProfile =
    script?.attached_profile ||
    script?.call_profile ||
    script?.conversation_profile ||
    null;
  return explicitProfile ? String(explicitProfile).trim() : null;
}

function getScriptProfileLabel(profileType) {
  if (!profileType) {
    return 'none';
  }
  return getCallScriptFlowLabel(profileType);
}

function getCallScriptSupportedProvidersLabel(scriptProfileRouting = {}) {
  if (scriptProfileRouting.primaryFlow === 'payment_collection') {
    return 'Twilio preferred';
  }
  return 'Configured call providers';
}

function buildCallScriptDiagnostics(
  script = {},
  scriptProfileRouting = resolveScriptProfileRouting(script)
) {
  const diagnostics = ['🧪 *Script diagnostics*'];
  const explicitProfile = getExplicitScriptAttachedProfile(script);
  const inferredProfile = !explicitProfile ? scriptProfileRouting.attachedProfile : null;
  const warnings = [...(scriptProfileRouting.warnings || [])];

  if (!explicitProfile && inferredProfile) {
    warnings.push(
      `Profile is inferred from flow metadata (${getScriptProfileLabel(inferredProfile)}) because the script does not declare an explicit attached profile.`
    );
  }

  diagnostics.push(
    `🧭 Primary flow: ${escapeMarkdown(getCallScriptFlowLabel(scriptProfileRouting.primaryFlow))}`
  );
  diagnostics.push(
    `🗂️ Flow types: ${escapeMarkdown(
      scriptProfileRouting.flowTypes.length
        ? scriptProfileRouting.flowTypes.map((flowType) => getCallScriptFlowLabel(flowType)).join(', ')
        : 'none'
    )}`
  );
  diagnostics.push(
    `🏷️ Objective tags: ${escapeMarkdown(
      scriptProfileRouting.objectiveTags.length ? scriptProfileRouting.objectiveTags.join(', ') : 'none'
    )}`
  );
  diagnostics.push(
    `🔒 Attached profile: ${escapeMarkdown(
      explicitProfile ? getScriptProfileLabel(explicitProfile) : 'none'
    )}`
  );
  diagnostics.push(
    `🧠 Inferred profile: ${escapeMarkdown(
      inferredProfile ? getScriptProfileLabel(inferredProfile) : 'none'
    )}`
  );
  diagnostics.push(
    `🔐 Lock state: ${escapeMarkdown(
      formatScriptProfileLockState(scriptProfileRouting.conversationProfileLock)
    )}`
  );
  diagnostics.push(
    `📡 Supported providers: ${escapeMarkdown(
      getCallScriptSupportedProvidersLabel(scriptProfileRouting)
    )}`
  );
  diagnostics.push(
    `⚠️ Warnings: ${escapeMarkdown(warnings.length ? warnings.join(' | ') : 'none')}`
  );
...
async function previewCallScript(conversation, ctx, script, ensureActive) {
  ...
  if (scriptProfileRouting.callProfile) {
    payload.call_profile = scriptProfileRouting.callProfile;
  }
  if (scriptProfileRouting.attachedProfile) {
    payload.attached_profile = scriptProfileRouting.attachedProfile;
  }
  if (scriptProfileRouting.autoAttachProfile !== null && scriptProfileRouting.autoAttachProfile !== undefined) {
    payload.auto_attach_profile = scriptProfileRouting.autoAttachProfile;
  }
  if (scriptProfileRouting.conversationProfile) {
    payload.conversation_profile = scriptProfileRouting.conversationProfile;
  }
  if (scriptProfileRouting.conversationProfileLock !== null && scriptProfileRouting.conversationProfileLock !== undefined) {
    payload.conversation_profile_lock = scriptProfileRouting.conversationProfileLock;
  }
  if (scriptProfileRouting.profileLockMode) {
    payload.profile_lock_mode = scriptProfileRouting.profileLockMode;
  }
  if (scriptProfileRouting.purpose) {
    payload.purpose = scriptProfileRouting.purpose;
  }

  await ctx.reply(buildCallScriptDiagnostics(script, scriptProfileRouting), {
    parse_mode: 'Markdown'
  });
...
async function createCallScriptFlow(conversation, ctx, ensureActive) {
  ...
  const isProfileFlow = PROFILE_FLOW_TYPE_SET.has(flowType);
  const starterTemplate = getCallScriptStarterTemplate(flowType);
  const profileFlowNotice = buildProfileFlowNotice(flowType);
  if (starterTemplate) {
    const flowLabel = getCallScriptFlowLabel(flowType);
    await ctx.reply(
      `🧩 Starter template loaded for ${flowLabel}. Use skip to keep the suggested description, prompt, and opening line.`
    );
  }
  if (profileFlowNotice) {
    await ctx.reply(profileFlowNotice);
  }
  ...
  const description = await promptText(
    ...
    '📝 Provide an optional description for this script (or type skip).',
    {
      allowEmpty: true,
      allowSkip: true,
      defaultValue: starterTemplate?.description || null,
      parse: (value) => value.trim(),
      ensureActive: safeEnsureActive
    }
  );
  ...
  const promptAndVoice = await collectPromptAndVoice(
    conversation,
    ctx,
    {
      prompt: starterTemplate?.prompt || null,
      first_message: starterTemplate?.first_message || null
    },
    safeEnsureActive
  );
  if (!promptAndVoice) {
  ...
  if (isProfileFlow) {
    const addCapture = await confirm(
      conversation,
      ctx,
      'Add optional digit capture settings to this profile-backed script?',
      safeEnsureActive
    );
    if (addCapture) {
  ...
  const objectiveTags = buildObjectiveTagsForFlow(flowType, []);
  const scriptProfileRouting = resolveScriptProfileRouting({
  ...
  const scriptPayload = {
    ...
    flow_type: flowType || null,
    objective_tags: objectiveTags,
    attached_profile: scriptProfileRouting.attachedProfile || null,
    auto_attach_profile: scriptProfileRouting.autoAttachProfile,
    profile_lock_mode: scriptProfileRouting.profileLockMode || null,
  };

  if (scriptProfileRouting.purpose && scriptPayload.persona_config) {
    scriptPayload.persona_config = {
      ...scriptPayload.persona_config,
      purpose: scriptProfileRouting.purpose
    };
  }
...
async function editCallScriptFlow(conversation, ctx, script, ensureActive) {
  ...
  if (adjustFlow) {
    ...
    const scriptProfileRouting = resolveScriptProfileRouting({
      flow_type: flowType,
      objective_tags: updates.objective_tags,
    });

    updates.attached_profile = scriptProfileRouting.attachedProfile || null;
    updates.auto_attach_profile = scriptProfileRouting.autoAttachProfile;
    updates.profile_lock_mode = scriptProfileRouting.profileLockMode || null;

    if (scriptProfileRouting.attachedProfile) {
      const flowNotice = buildProfileFlowNotice(flowType);
      if (flowNotice) {
